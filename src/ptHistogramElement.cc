...
HistogramElement::HistogramElement(const DataSpec cds,
  ...
  :dataPlotElement(cds, layout, field, xtime),
   ...
   lineWidth(layout.lineWidth),deltaY(field.y2-startY-layout.labelSpace),
   unit(layout.unit),hstart(layout.histStart),hstop(layout.histStop),
   drawlabel(layout.label)
{
...
#ifdef DEBUG
      cout << "Histogram value:" << dval(j)
	   << " deltaT:" << deltaT[i] << endl;
#endif
  ...
  if (nt > 1) deltaT[0]= deltaT[1];
  else deltaT[0]= 1;
#ifdef DEBUG
  cout << "Histogram constructor: max:"<<max<<endl;
  cout << "deltaT:"<<endl;
  for (int i=0; i<nt;i++) cout<<deltaT[i]<<endl;
#endif
}
