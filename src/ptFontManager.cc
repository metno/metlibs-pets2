...
FontManager::~FontManager(){
  for (int i=0; i<MAXFONTCOLL; i++)
    if (FC[i].fontc) delete FC[i].fontc;
}
...
bool FontManager::addFontCollection(glText* fc,
				    const miString s)
{
  if (!s.exists() || !fc) return false;
  for (int i=0; i<MAXFONTCOLL; i++)
    if (!FC[i].fontc){
      FC[i].name= s;
      FC[i].fontc = fc;
...
ptFontSize FontManager::getFontSize()
{
  ...
  if (curFC>=0){
    ...
    if (fs==HEADER1_size)        pfs= HEADER1;
    else if (fs==HEADER2_size)   pfs= HEADER2;
    else if (fs==HEADER3_size)   pfs= HEADER3;
    else if (fs==NORMAL_size)    pfs= NORMAL;
    else if (fs==SUBSCRIPT_size) pfs= SUBSCRIPT;
  }
  return pfs;
}
...
int FontManager::setFontSize(const ptFontSize size)
{
  if (curFC>=0){
    ...
    if (size==HEADER1)        fs= HEADER1_size;
    else if (size==HEADER2)   fs= HEADER2_size;
    else if (size==HEADER3)   fs= HEADER3_size;
    else if (size==NORMAL)    fs= NORMAL_size;
    else if (size==SUBSCRIPT) fs= SUBSCRIPT_size;
...
// printing commands
int FontManager::printChar(const int c,
...
int FontManager::printStr(const char* s,
			  const float x, const float y)
{
  if (curFC>=0)
