...
BoxElement::BoxElement(const DataSpec cds,
  ...
  :dataPlotElement(cds, layout, field, xtime),
   label(layout.text), linewidth(layout.lineWidth),
   pattern(layout.fillstyle),
   colorlist(layout.colorlist), patternlist(layout.patternlist),
   hsides(layout.horsides), vsides(layout.versides),
   useTimes(layout.useTimes),timeLine(tline),style(layout.linePattern)
{
#ifdef DEBUG
  ...
  if(enabled && visible) {
	...
	if (ndat!=ldat || i==stopT){
	  if (ldat>=0 && ldat<colorlist.size()){
	    ...
	    glRectf(xtime->xcoord[lidx], startY,
		    xtime->xcoord[nidx], stopY);
	  }
	  ldat= ndat;
	  lidx= nidx;
	}
