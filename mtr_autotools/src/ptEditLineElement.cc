...
EditLineElement::EditLineElement(yAxisElement* ya,
				 const DataSpec cds,
				 const ptVertFieldf& field, 
				 const Layout& layout,
				 XAxisInfo* xtime)
  : AxisChildElement(ya,cds,field,layout,xtime),
    editstyle(layout.editstyle), fstyle(layout.fillstyle),
    pointAlign(layout.align), printValueLock(layout.printValueLock),
	...
	if (editstyle == ES_HISTOGRAM){   // HISTOGRAM
	  if (pointAlign == LEFT){ // pointAlign == LEFT
	    ...
	    if ((dataY[k] - startY) > 0){
	      ...
	      if (lineWidth > 0){
		glBegin(GL_LINE_STRIP);
		glVertex2f(x1,startY); glVertex2f(x1,dataY[k]);
		glVertex2f(x2,dataY[k]); glVertex2f(x2,startY);
		glEnd();
	      }
	    }
		...
		if ((oldy - startY) > 0){
		  ...
		  if (lineWidth > 0){
		    glBegin(GL_LINE_STRIP);
		    glVertex2f(x1,startY); glVertex2f(x1,oldy);
		    glVertex2f(x2,oldy); glVertex2f(x2,startY);
		    glEnd();
		  }
		}
		drawprevhisto= false;
...
// -------------------------------------------------------------------------
bool EditLineElement::grabPoint(float x, float y, bool mark,
				bool fillInterval)
{
  ...
  if (quantized) _resetAccu(); // reset accumulated edit-values
  l=datastart();
  int dstart= l;
  for (i = startT; i<=stopT; i++) {
    if (valid(i)) {
	...
	if (mark) { // marking on, toggle flag..
	  ...
	  if (markedPoints[l] && fillInterval) { // if marking turned on, check neighbours
	    ...
	    if (l>0) { // check points to the left
	      for (j = l-1; j>=0; j--)
		if (markedPoints[j]) {
		  for (k = j; k < l; k++) markedPoints[k] = true;
		  break;
		}
	    }
 ...
 */
bool EditLineElement::grabPoint(int  step,
				bool mark,
				bool fillInterval)
{
  if (!activenodes) return false;
  ...
  if ( step == 0 ){
    if ( mark ) markedPoints[activePoint] = true;
    return true;
  }
...
{
  if (!printValueLock) printValue = flag;
}
...
{
  if (quantum<0.00001) return false;
...
void EditLineElement::movePoints(float dy)
{
  ...
  case LE_MULTI:
    ...
    break;
  case LE_ROTATE:
    if ((activePoint==xtime->xcoord.size()-1 || !markedPoints[activePoint+1])
	&& activePoint>0)
      {
