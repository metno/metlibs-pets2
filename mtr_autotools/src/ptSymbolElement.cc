...
SymbolElement::SymbolElement(const DataSpec cds,
			     const ptVertFieldf& field, 
			     const Layout& layout,  
			     XAxisInfo* xtime)
  :dataPlotElement(cds, layout, field, xtime),
   minidx(0),maxidx(0),maxwidth(1),maxheight(1),
...
void SymbolElement::setImages(const int min,
			      const vector<miString>& files)
{
  int i;
  minidx= min;
...
void SymbolElement::plot()
{
  if(enabled && visible) {
	...
	if (dval(j)>=minidx && dval(j)<=maxidx){
	  ...
	  images[index].plot(xtime->xcoord[i]-halfw,
			     startY,scale,
			     pixWidth,pixHeight,psoutput,true);
 	  _updatePrinting();
	}
	j++;
      }
