...
stMother Str2Mother(miString buf){
  ...
  if (mother=="AXES1")          return AXES1;
  else if (mother=="AXES2")     return AXES2;
  else if (mother=="AXES3")     return AXES3;
  else if (mother=="AXES4")     return AXES4;
  else if (mother=="AXES5")     return AXES5;
  else if (mother=="DIAGRAM")   return DIAGRAM;
  else if (mother=="NO_MOTHER") return NO_MOTHER;
  else return NO_MOTHER;
}
...
Primitive::Primitive(ptPrimitiveType t)
  :type(t), order(-1), id(ID_UNDEF), nr(0), conv(NO_CONVERSION),
   mother(NO_MOTHER), plotAll(false), enabled(true), component(0)
{
}
...
//   stMother curMother;
  ...
  for (curMother=AXES1; curMother<=maxMother; index+=motherCount[curMother++])
...
// 	}
 	if (pprimF[iprim].type == YAXIS ||
	    pprimF[iprim].type == YAXIS_STATIC) {
	  usedAxes[pprimF[iprim].mother][pprimF[iprim].layout.axis] = true;
	}
...
bool ptStyle::readStyle(const miString filename,
			bool verbose){
  ...
  while (sfile.good()){
    ...
    else if (keyw=="bottommargin") bottomMargin_= atof(argu.cStr());
    else if (keyw=="leftmargin")   leftMargin_= atof(argu.cStr());
    else if (keyw=="rightmargin")  rightMargin_= atof(argu.cStr());
    else if (keyw=="localtime")  localTime= (argu.upcase()=="TRUE");
    else if (keyw=="timezone")  timeZone= atoi(argu.cStr());
    else if (keyw=="leftoffset") leftOffset_= atof(argu.cStr());
    else if (keyw=="rightoffset") rightOffset_= atof(argu.cStr());
    // primitive-attributes
    else if (keyw=="type") curp.type= Str2Primitive(argu);
    else if (keyw=="order") curp.order= atoi(argu.cStr());
    else if (keyw=="enabled") curp.enabled= (argu.upcase()=="TRUE");
    else if (keyw=="mother") curp.mother= Str2Mother(argu);
    else if (keyw=="component") curp.component= atoi(argu.cStr());
    else if (keyw=="parameter") curp.id= parDef.Str2ParId(argu);
    else if (keyw=="alias") curp.id.alias= argu;
    else if (keyw=="level") curp.id.level= atoi(argu.cStr());
    else if (keyw=="model") curp.id.model= argu;
    else if (keyw=="run") curp.id.run= atoi(argu.cStr());
    else if (keyw=="submodel") curp.id.submodel= argu;
    else if (keyw=="plotall")  curp.plotAll= (argu.upcase()=="TRUE");
    // layout-attributes
    else if (keyw=="name") curl.name= argu;
    else if (keyw=="visible") curl.visible= (argu.upcase()=="TRUE");
    else if (keyw=="color") curl.color.fromStr(argu);
    else if (keyw=="color2") curl.color2.fromStr(argu);
    else if (keyw=="color3") curl.color3.fromStr(argu);
    else if (keyw=="colorlist") curl.colorlist= ptColor::Str2Colorlist(argu);
    else if (keyw=="font") curl.font= Str2FontSize(argu);
    else if (keyw=="height") curl.height= atoi(argu.cStr());
    else if (keyw=="trueheight") curl.trueheight= atoi(argu.cStr());
    else if (keyw=="position") curl.position= atoi(argu.cStr());
    else if (keyw=="size") curl.size= atoi(argu.cStr());
    else if (keyw=="marker") curl.marker= Str2Marker(argu);
    else if (keyw=="markerfill") curl.markerFill= Str2FillStyle(argu);
    else if (keyw=="rectangle") curl.axisRectangle= (argu.upcase()=="TRUE");
    else if (keyw=="linepattern") curl.linePattern= Str2LineStyle(argu);
    else if (keyw=="fillpattern") curl.fillstyle= Str2FillStyle(argu);
    else if (keyw=="patternlist") curl.patternlist= Str2FillStyleList(argu);
    else if (keyw=="linepatternlist") curl.linepatternlist= Str2LineStyleList(argu);
    else if (keyw=="align") curl.align= Str2Align(argu);
    else if (keyw=="spacing") curl.spacing= Str2Size(argu);
    else if (keyw=="intspacing") curl.intSpacing= Str2Size(argu);
    else if (keyw=="label") curl.label= (argu.upcase()=="TRUE");
    else if (keyw=="patternincolour")
      curl.patternInColour= (argu.upcase()=="TRUE");
    else if (keyw=="linewidth") curl.lineWidth= atof(argu.cStr());
    else if (keyw=="axiswidth") curl.axisWidth= atof(argu.cStr());
    else if (keyw=="tickwidth") curl.tickWidth= atof(argu.cStr());
    else if (keyw=="ticklen") curl.tickLen= atof(argu.cStr());
    else if (keyw=="minrange") curl.minRange= atof(argu.cStr());
    else if (keyw=="interval") curl.interval= atof(argu.cStr());
    else if (keyw=="delta") curl.delta= atof(argu.cStr());
    else if (keyw=="minmargin") curl.minMargin= atof(argu.cStr());
    else if (keyw=="maxisset") curl.maxIsSet= (argu.upcase()=="TRUE");
    ...
    else if (keyw=="vectorarrow") curl.vectorArrow= (argu.upcase()=="TRUE");
    else if (keyw=="linearrows") curl.lineArrows= (argu.upcase()=="TRUE");
    else if (keyw=="arrowlength") curl.arrowLength= atof(argu.cStr());
    else if (keyw=="arrowsize") curl.arrowSize= atof(argu.cStr());
    else if (keyw=="numtickmajor") curl.numTickMajor= atoi(argu.cStr());
    else if (keyw=="numtickminor") curl.numTickMinor= atoi(argu.cStr());
    else if (keyw=="labelspace") curl.labelSpace= atof(argu.cStr());
    else if (keyw=="quantized") curl.quantized= (argu.upcase()=="TRUE");
    else if (keyw=="quantum") curl.quantum= atof(argu.cStr());
    else if (keyw=="gridxonly") curl.gridxonly= (argu.upcase()=="TRUE");
    else if (keyw=="plotdaypattern") curl.plotDayPattern= (argu.upcase()=="TRUE");
    else if (keyw=="axisgrid") curl.axisgrid= (argu.upcase()=="TRUE");
    else if (keyw=="gridwidth") curl.gridwidth= atof(argu.cStr());
    else if (keyw=="gridcolor") curl.gridcolor.fromStr(argu);
    else if (keyw=="gridstyle") curl.gridstyle= Str2LineStyle(argu);
    else if (keyw=="minskipx") curl.minSkipX= atoi(argu.cStr());
    else if (keyw=="usetimes") curl.useTimes= atoi(argu.cStr());
    else if (keyw=="modhours") curl.modhours= Str2IntList(argu.cStr());
    else if (keyw=="axis") curl.axis= Str2Axis(argu);
    else if (keyw=="text") curl.text= argu;
    else if (keyw=="text2") curl.text2= argu;
    else if (keyw=="unit") curl.unit= (argu.upcase()=="TRUE");
    else if (keyw=="pformat") curl.pformat= argu;
    else if (keyw=="vectorf") curl.vectorF= (argu.upcase()=="TRUE");
    else if (keyw=="linelabelpos") curl.lineLabelPos= atof(argu.cStr());
    else if (keyw=="labelonline") curl.labelOnLine= (argu.upcase()=="TRUE");
    else if (keyw=="keepinaxis") curl.keepinaxis= (argu.upcase()=="TRUE");
    else if (keyw=="shadow") curl.shadow= (argu.upcase()=="TRUE");
    else if (keyw=="histogramstart") curl.histStart= atoi(argu.cStr());
    else if (keyw=="histogramstop") curl.histStop= atoi(argu.cStr());
    else if (keyw=="fittopage") curl.fittopage= (argu.upcase()=="TRUE");
    else if (keyw=="horlabels") curl.horLabels= (argu.upcase()=="TRUE");
    else if (keyw=="horlabeloffset") curl.horLabelOffset= atof(argu.cStr());
    else if (keyw=="cutoff") curl.cutoff= atof(argu.cStr());
    else if (keyw=="horsides") curl.horsides= Str2HorSides(argu);
    else if (keyw=="versides") curl.versides= Str2VerSides(argu);
    else if (keyw=="priority") curl.priority= atoi(argu.cStr());
    else if (keyw=="language") curl.language= argu.upcase();
    ...
    else if (keyw=="asnumber") curl.asNumber= (argu.upcase()=="TRUE");
    else if (keyw=="datestyle") curl.datestyle= Str2DateStyle(argu);
    else if (keyw=="legendlinesinside") curl.legendlineinside= (argu.upcase()=="TRUE");
    else if (keyw=="datalimits") curl.datalimits= Str2FloatList(argu);
    else if (keyw=="colorbyvalue") curl.colorbyvalue= (argu.upcase()=="TRUE");
    else if (keyw=="linestylebyvalue") curl.linestylebyvalue= (argu.upcase()=="TRUE");
    else if (keyw=="fillbyvalue") curl.fillbyvalue= (argu.upcase()=="TRUE");
    else if (keyw=="smoothing") curl.smoothing= (argu.upcase()=="TRUE");
